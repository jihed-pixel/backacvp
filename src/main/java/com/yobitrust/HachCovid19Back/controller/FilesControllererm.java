...
import com.yobitrust.HachCovid19Back.service3.FilesStorageService3;
...
@Controller
@CrossOrigin("https://projet-48fcf.web.app")
public class FilesControllererm {
  ...
  @CrossOrigin(origins ="https://projet-48fcf.web.app" )
  @GetMapping("/files3")
  public ResponseEntity<List<FileInfo>> getListFiles() {
    List<FileInfo> fileInfos = storageService.loadAll3().map(path -> {
      String filename = path.getFileName().toString();
      String url = MvcUriComponentsBuilder
          .fromMethodName(FilesControllererm.class, "getFile", path.getFileName().toString()).build().toString();
